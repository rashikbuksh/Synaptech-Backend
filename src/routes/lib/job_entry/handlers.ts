...
import { job, job_entry, product, product_serial, vendor } from '../schema';
...
export const remove: AppRouteHandler<RemoveRoute> = async (c: any) => {
  ...
  const _data2 = await db.delete(product_serial)
    .where(eq(product_serial.job_entry_uuid, uuid))
    .returning({
